...
class BoardController: NSObject,
  ...
  func deleteLastCharacter() {
    ...
    // START YOUR CODE HERE
      if let lastCell = collectionView.cellForItem(at: IndexPath(item: numGuesses - 1, section: 0)) as? LetterCell {
          // Clear the letter from the cell
          lastCell.clearLetter()
          
          // Set the cell's style to initial
          lastCell.set(style: .initial)
